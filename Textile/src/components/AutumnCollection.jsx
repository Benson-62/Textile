...
const AutumnCollection = () => {
  ...
  // Fetch autumn collection products
  const fetchAutumnCollections = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/autumn-collection');
  ...
  const handleSaveProduct = async () => {
    try {
      if (editItem) {
        await axios.put(`http://localhost:5000/api/autumn-collection/${editItem.id}`, newProduct);
      } else {
        await axios.post('http://localhost:5000/api/autumn-collection', newProduct);
      }
      fetchAutumnCollections();
      handleClose();
    } catch (error) {
  ...
  const handleDeleteProduct = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/api/autumn-collection/${id}`);
      fetchAutumnCollections();
    } catch (error) {
  ...
  return (
    <Box sx={{ display: 'flex', padding: 4, position: 'relative' }}>
      ...
      <Box sx={{ flex: 1, paddingRight: 4 }}>
        <Typography variant="h4" sx={{ marginBottom: 2 }}>Autumn Collection</Typography>
