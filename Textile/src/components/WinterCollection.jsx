...
const WinterCollection = () => {
  ...
  // Fetch winter collection products
  const fetchWinterCollections = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/winter-collection');
  ...
  const handleSaveProduct = async () => {
    try {
      if (editItem) {
        await axios.put(`http://localhost:5000/api/winter-collection/${editItem.id}`, newProduct);
      } else {
        await axios.post('http://localhost:5000/api/winter-collection', newProduct);
      }
      fetchWinterCollections();
      handleClose();
    } catch (error) {
  ...
  const handleDeleteProduct = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/api/winter-collection/${id}`);
      fetchWinterCollections();
    } catch (error) {
  ...
  return (
    <Box sx={{ display: 'flex', padding: 4, position: 'relative' }}>
      ...
      <Box sx={{ flex: 1, paddingRight: 4 }}>
        <Typography variant="h4" sx={{ marginBottom: 2 }}>Winter Collection</Typography>
